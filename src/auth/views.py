...
from .forms import ResetPasswordForm, EmailForm, LoginForm, RegistrationForm, FileUploadForm
from ..data.database import db
from ..data.models import User, UserPasswordToken,Products
...
from .parse_xlsx import getdata_vskprofi
...
def upload():
    ...
    if form.validate_on_submit():
        ...
        #product=[]
        for row in getdata_vskprofi(file_path):
            product = Products.find_by_UID(row['UID'])
            if product:
                product.UID=row['UID']
                product.order_number=row['order_number']
                product.ShortName=row['ShortName']
                product.Description=row['Description']
                product.Quantity=0
                product.Groups = row['groups']
                product.UnitType=row['UnitType']
                product.update()
